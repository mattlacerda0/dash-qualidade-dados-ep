...
export function getCoreEnv() {
  const url = trimUrl(process.env.CORE_SUPABASE_URL || process.env.PHARUS_SUPABASE_URL);
  const serviceRoleKey = trimEnv(
...
export function coreConfigurationError() {
  const { url, serviceRoleKey } = getCoreEnv();
  if (!url || !serviceRoleKey) {
    return "Configure CORE_SUPABASE_URL e CORE_SUPABASE_SERVICE_ROLE_KEY ou use PHARUS_SUPABASE_URL e PHARUS_SUPABASE_ANON_KEY.";
  }
  if (!/^https:\/\//i.test(url)) {
    return "CORE_SUPABASE_URL deve usar HTTPS.";
  }
  return null;
}
