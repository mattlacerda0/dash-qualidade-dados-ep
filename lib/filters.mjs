...
function resolveCatalog(fields) {
  return Array.isArray(fields) ? fields : EP_UNFILLED_CATALOG;
}

export function visibleCatalogFields(domain = "all", fields) {
  const selected = normalizeEpUnfilledDomain(domain);
  const catalog = resolveCatalog(fields);
  if (selected === "all") return catalog;
  return catalog.filter((field) => field.domain === selected);
}

export function catalogForLocalFilters(localFilters = {}, fields) {
  ...
  let scoped = visibleCatalogFields(domain, fields);
  if (fieldId !== "all") scoped = scoped.filter((field) => field.id === fieldId);
  return scoped;
}

export function fieldSelectOptions(domain = "all", fields) {
  return visibleCatalogFields(domain, fields).map((field) => ({ value: field.id, label: field.label }));
}
...
export function summarizeEpUnfilled(clients = [], fields) {
  const catalog = visibleCatalogFields("all", fields);
...
export function summarizeFieldTable(clients = [], localFilters = {}, fields) {
  const next = { ...defaultTableLocalFilters(), ...localFilters };
  ...
  const catalog = catalogForLocalFilters(next, fields);
  const severity = normalizeEpUnfilledSeverity(next.severity);
  return sortFieldRows(catalog.map((field) => fieldCoverage(scoped, field))).filter(
    (row) => severity === "all" || row.severity === severity,
  );
}

export function summarizePriorityFields(clients = [], fields) {
  const catalog = resolveCatalog(fields);
  const byId = new Map(catalog.map((field) => [field.id, field]));
  const priority = EP_UNFILLED_PRIORITY_FIELD_IDS.map((id) => byId.get(id)).filter(Boolean);
  return sortFieldRows(priority.map((field) => fieldCoverage(clients, field)));
}

export function summarizeEmptyClients(clients = [], localFilters = {}, fields) {
  const next = { ...defaultTableLocalFilters(), ...localFilters };
  const engineer = next.engineer && next.engineer !== "all" ? String(next.engineer) : "all";
  const catalog = catalogForLocalFilters(next, fields);
  const scoped = engineer === "all"
  ...
  return scoped
    .map((client) => {
      ...
      const fillPercent = catalog.length ? round1((filled / catalog.length) * 100) : 0;
      return {
...
export function summarizeEpTable(clients = [], localFilters = {}, fields) {
  const next = { ...defaultTableLocalFilters(), ...localFilters };
  const catalog = catalogForLocalFilters(next, fields);
  const engineer = next.engineer && next.engineer !== "all" ? String(next.engineer) : "all";
  const severity = normalizeEpUnfilledSeverity(next.severity);
  ...
  let engineers = sortEngineerRows(
    [...grouped.entries()].map(([name, rows]) => engineerCompleteness(name, rows, catalog)),
  );
